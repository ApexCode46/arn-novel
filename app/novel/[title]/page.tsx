    ...
    <>
      <div className="container mx-auto px-4 md:px-8 lg:px-16 xl:px-32 2xl:px-64 py-4">
        ...
        <div className="my-4">
          ...
          <ScrollArea className="h-96 w-full bg-backgroundCustom drop-shadow-md border rounded">
            <div className="p-4">
              <h4 className="mb-4 text-sm font-medium leading-none">ตอนที่ {pageOptions.find(option => option.value === currentPage)?.label.split(' ')[1]}</h4>
              {currentChapters.map((chapter) => (
                <div key={chapter.id} className="text-sm my-2 p-4 bg-background rounded hover:bg-gray-100 cursor-pointer transition-colors">
                  {chapter.title}
                </div>
              ))}
            </div>
